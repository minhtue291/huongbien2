...
export default function StaffOrder({
    ...
    <div className="flex flex-col gap-3 pb-20 max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-2 gap-2 bg-slate-100 p-1.5 rounded-2xl">
        ...
        <button
          ...
          className={`py-2.5 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all relative ${
            activeTab === 'cart'
              ? 'bg-sky-600 text-white shadow-xs'
              : 'text-slate-600 hover:bg-slate-200/60'
          }`}
        >
          ...
          {currentTableCart.length > 0 && (
            <span className={`ml-1 px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${
              activeTab === 'cart' ? 'bg-white text-sky-600' : 'bg-rose-500 text-white'
            }`}>
                  ...
                  return (
                    <div 
                      ...
                      className={`p-3 rounded-xl border transition-all ${
                        isPrinted ? 'bg-slate-50 border-slate-200' : 'bg-amber-50/70 border-amber-300'
                      }`}
                    >
  ...
  className={`py-2.5 px-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-2xs ${
    hasNewItemsToPrint 
